...
def create_horizontal_bar(df, y_col, value_col, color, height=250, top_n=None):
    ...
    fig.update_yaxes(tickfont=dict(size=12 if not top_n else 11))
    return fig


def create_top_n_bar(df, col_name, color, top_n=15, height=250):
    counts = df[col_name].value_counts().head(top_n).iloc[::-1].reset_index()
...
def create_parent_job_bar(df, color, top_n=15, height=300):
    ...
    cleaned = combined[combined["Job"].notna() & (combined["Job"] != "Không có thông tin")]
